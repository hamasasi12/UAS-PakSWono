...
function Mahasiswa() {
  ...
  const handleDeleteData = (no) => {
    fetch(`http://localhost:5000/api/permintaanmasuk/${no}`, {
      method: 'DELETE',
      headers: {
  ...
  const handleSubmit = (e) => {
    ...
    const url = isEditing ? `http://localhost:5000/api/permintaanmasuk/${formData.no}` : 'http://localhost:5000/api/permintaanmasuk';
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5000/api/permintaanmasuk', {
      headers : {
